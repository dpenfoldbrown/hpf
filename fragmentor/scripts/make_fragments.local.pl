...
# PSIPRED_DIR
# NNMAKE_SHORT_DIR - nnmake_short scripts
# JUFO_DIR
# SAM_DIR
# SAM_2ND_DIR - SAM Secondary Structure Prediction

local $BLAST_DIR = $ENV{"BLAST_DIR"};
local $BLASTBIN = "$BLAST_DIR/bin";
local $NR_DIR = $ENV{"NR_DIR"};
local $NNMAKEDB_DIR = $ENV{"NNMAKEDB_DIR"};      # BLOSUM score matrices directory (cvs repository 'nnmake_database')
local $PSIPRED_DIR = $ENV{"PSIPRED_DIR"};
local $PSIPREDBIN = "$PSIPRED_DIR/bin";
local $NNMAKE_SHORT_DIR = $ENV{"NNMAKE_SHORT_DIR"};
local $JUFO_DIR = $ENV{"JUFO_DIR"};
local $SAM_DIR = $ENV{"SAM_DIR"};
local $SAMBIN = "$SAM_DIR/bin";
local $SAM_2ND_DIR = $ENV{"SAM_2ND_DIR"};

# tail of fragment files
local $TAIL = "_v1_3";

# psi-blast
local $PSIBLAST = "$BLASTBIN/blastpgp";
local $VALL_BLAST_DB = "$NNMAKEDB_DIR/vall.blast.2006-05-05";
local $VALL_NAME = "vall.dat.2006-05-05";

# nr filter
local $NR = "$NR_DIR/nr";
local $FILTNR = "$NR";        # filtnr blast database filename

# psipred
local $MAKEMAT = "$BLASTBIN/makemat";       # makemat utility (part of NCBI tools)
local $PSIPRED = "$PSIPREDBIN/psipred";       # psipred
local $PSIPASS2 = "$PSIPREDBIN/psipass2";     # psipass2 (part of psipred pkg)
local $PSIPRED_DATA = "$PSIPRED_DIR/data";         # dir containing psipred data files.
 
# nnmake
local $BLOSUM = $NNMAKEDB_DIR;
local $VALL = $BLOSUM;          # dir containing vall database (cvs repository 'nnmake_database')
local $VALL2 = $BLOSUM;             # alt dir containing vall database (cvs repository 'nnmake_database')
#local $NNMAKE = "$NNMAKE_SHORT_DIR/pNNMAKESHORT.gnu";
local $NNMAKE = "$NNMAKE_SHORT_DIR/pNNMAKE.gnu";
...
local $JUFO = "$JUFO_DIR/molecule.exe";                  # jufo executable

# sam (secondary structure prediction software)
local $SAM_target99		= "$SAMBIN/bin/target99";		# sam target99 executable
local $SAM_uniqueseq		= "$SAMBIN/uniqueseq";		# sam uniqueseq executable
local $SAM_predict_2nd_dir	= "$SAM_2ND_DIR/";			# sam predict-2nd directory
local $SAM_predict_2nd		= "$SAM_2ND_DIR/predict-2nd";	# sam predict-2nd executable
local $SAM_condense_rdb		= "$SAM_2ND_DIR/condense_rdb.pl";	# condense 6 to 3 state executable
...
unless (&fileExist("$id$chain.check" ))  {
  if (!&try_try_again("$PSIBLAST -i $file -F F -j2 -o $id$chain.blast -d $NR -v2000 -b2000 -K2000 -h0.0009 -e0.0009 -C $id$chain.check -Q $id$chain.pssm",
...
if ($jufo) {
    ...
    #DEBUG - don't clean up jufo input/output
...
#DEBUG - don't remove path_defs.txt file
