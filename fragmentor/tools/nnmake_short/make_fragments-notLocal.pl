...
local $NR = "/data/seqdb/blastformat/nr";                # nr blast database filename
...
local $BLOSUM = "/users/rbonneau/pred/nnmake_database/";      # BLOSUM score matrices directory (cvs repository 'nnmake_database')
 
# psipred
local $FILTNR = "/users/rbonneau/pred/nnmake_database/filtnr/filtnr";        # filtnr blast database filename
local $MAKEMAT = "/package/genome/bin/makemat";       # makemat utility (part of NCBI tools)
local $PSIPRED = "/users/rbonneau/sspred/psipred/bin/psipred";       # psipred
local $PSIPASS2 = "/users/rbonneau/sspred/psipred/bin/psipass2";     # psipass2 (part of psipred pkg)
local $PSIPRED_DATA = "/users/rbonneau/sspred/psipred/data";         # dir containing psipred data files.
 
# nnmake
local $VALL = "/users/rbonneau/pred/nnmake_database";          # dir containing vall database (cvs repository 'nnmake_database')
local $VALL2 = "/users/rbonneau/pred/nnmake_database";             # alt dir containing vall database (cvs repository 'nnmake_database')
...
local $JUFO = "/users/rbonneau/sspred/jufo/molecule.exe";                  # jufo executable

# sam (secondary structure prediction software)
local $SAM_target99		= "/net/local/sam/bin/target99";		# sam target99 executable
local $SAM_uniqueseq		= "/net/local/sam/bin/uniqueseq";		# sam uniqueseq executable
local $SAM_predict_2nd_dir	= "/net/local/sam.predict2nd/";			# sam predict-2nd directory
local $SAM_predict_2nd		= "/net/local/sam.predict2nd/predict-2nd";	# sam predict-2nd executable
local $SAM_condense_rdb		= "/net/local/sam.predict2nd/condense_rdb.pl";	# condense 6 to 3 state executable
