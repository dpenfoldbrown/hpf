...
c---------------------------------------------------------------
      subroutine dipolar_set_updatetensor(flag)

      implicit none
      
      logical flag

      logical update,old_update
      common /dipolar_uflag/ update,old_update
      data update /.true./    !initial value
...
c------------------------------------------------------------------------
      ...
      subroutine read_dipolar(filename,iunit,exist)
...
car initialize values of tensor and reject flags:
 ...
 1000 write(0,*)"NNMAKE FAILED: format error in .dpl file comment lines 1-3"
...
car differs for rosetta scheme
      ...
      if (atom.eq.'HN  ') then
         atomlookup = 0
         return
      elseif (atom.eq.'HA  ' .or. atom.eq.'HA1 ') then
         atomlookup = -1
         return
      elseif (atom.eq.'HA2 ') then
         atomlookup = -2
         return
      ...
      elseif (atom.eq.' C  '.or.atom.eq.'C ') then !C'=atom 4 
         ...
         write(0,*) "Atom type: ",atom," unknown in read_dipolar.f"
         stop
      endif
...
c-----------------------------------------------------------------------------
      subroutine eval_dipolar(xyz,first,last,score)
...
car 
car Input data include the first and last residues over which the score
car is to be calculated. These should correspond to the numbering
car scheme in the input dipolar constraints file. Note that dipolar
car couplings involving the HN of residue first are not included in the
car score because the orientation of the HN bond vector is not defined
car unless the phi of residue first is defined.
car
car Input xyz contains backbone coordinates for residues first 
car through last. Coordinates are assumed to dimensioned for five atoms
car per residue.
car
...
car
car References:
car  Losonzi et al. (1999) JMR 138:334
...
car input
      real xyz(3,5,*)           !coordinates for first to last residues only
      integer first,last        !first and last residues over which
...
car      13C-15N
car      13C-13C
car If more are added, need to adjust function Dmax accordingly
...
car internal 
      ...
      integer ndata(MAXDIPSETS)      !number of rows with real data
      real setscore
      ...
      integer map(MAXDIPOLAR)   !Jdipolar(map(i))=coupling constant of row(i)
                                !index to access complete arrays

      real*8 x(ORDERSIZE,MAXDIPSETS)   !independent elements of order matrix 
                                       !(Syy,Szz,Sxy,Sxz,Syz)
      real*8 Azz(MAXDIPSETS)   !principal order parameter of diag align tensor
      real*8 eta(MAXDIPSETS)     !rhombicity
      real*8  paf_rot(3,3,MAXDIPSETS)  !princ. align. frame. rotation matrix
                                        
      logical reject(MAXDIPSETS)    !true if an ordermatrix can't be determined
...
car pairDipolar(1,i) .le.  pairDipolar(1,i+1) 
      ...
      if (verbose) write(dunit,*)"residue, Jobs, Jcalc, residual**2"
      do iset=1,ndipolar_sets
         if (.not.update .and. reject(iset)) then
            write(0,*)'tensor not meaningful, must be updated'
            score=1e9
            return
         endif
         if (verbose) write(dunit,*)"dipolar constraint set ",iset
         call assemble_datamatrix(xyz,first,last,iset,A,b,nrow,map)
         ndata(iset)=nrow 
         if (update) then
     ...
     #           eta(iset))
         ...
         call calc_dipscore(A,x(1,iset),b,map,ndata(iset),
     #        Azz(iset),setscore)  
	 if (verbose) then 
            write(dunit,*)'Azz', Azz(iset)
            write(dunit,*)'eta', eta(iset)
            write(dunit,*)'x',(x(i,iset),i=1,5)
	 endif
         score=score+setscore
 300     continue         
      enddo

      score=score* Ndipolar !(last-first)+1
      
      if (verbose) write(dunit,*)"total dipolar score: ",score
      call dipolar_set_verbose(.false.)
      return
      end
...
c--------------------------------------------------------------------------
      subroutine assemble_datamatrix(xyz,first,last,iset,A,b,nrow,map)
...
car internal      
      integer i
      real temp
      real m(3),n(3)           !coordinates of coupled atoms
...
car function
      ...
      nrow =0
      do i=1,ndipolar 
        if (pairDipolar(1,i).gt.last) return
         if (pairDipolar(5,i).ne.iset) goto 200 !wrong set
     ...
     #        goto 200          !HN of first not determined
         ...
         nrow=nrow+1            !acceptable constraint
         map(nrow)=i

         call get_atomxyz(xyz,pairdipolar(1,i)-first+1,
     #        pairdipolar(2,i),m)
         call get_atomxyz(xyz,pairdipolar(3,i)-first+1,
     #        pairdipolar(4,i),n)
...
car cos(thetax)=umn(x)     i.e. x component of unm
         ...
         umn(2)=m(2)-n(2)
         umn(3)=m(3)-n(3)
         if (dipolartype(pairdipolar(1,i)).le. 5) then 
            r = Ddist(i)  
...
c-----------------------------------------------------------------------
      ...
      subroutine fix_substructure(position,nres,fixresidue)
      
      implicit none

      integer nres
      real position(3,5,nres)
      logical fixresidue(nres) 

      integer window
      parameter (window=25)
      real cutoff
      parameter (cutoff=8.0)
      integer minlength
...
!get the alignment tensor for the whole protein
      call dipolar_set_updatetensor(.true.)
      call eval_dipolar(position,1,nres,score)

!score the protein over windows
      last=nres-window+1
      write(0,*)last,nres
      count=0
      start=0
      bestcount=0
      beststart=nres+1
      call dipolar_set_updatetensor(.false.)
      do i=1,last
         call eval_dipolar(position(1,1,i),i,i+window-1,score)
         if (score.lt.cutoff) then    !in low score region
            if (count.eq.0) start = i       !start new low score region
            count=count+1
            if (count.gt.bestcount) then
               bestcount=count
               beststart=start
            endif
         else                             !not in low score region
            count=0
         endif
         write(0,*)i,score,start,count,beststart,bestcount
      enddo
      
      call dipolar_set_updatetensor(.true.)
c      if (bestcount=0) return
c      call eval_dipolar(position(1,1,beststart),i,i+window+bestcount-2,
c     #     score,.false.)
      if (bestcount.lt.minlength) then
         write(0,*)"NO RESIDUES FIXED"
         return
      endif

      do i=1,nres
         if (i.ge.beststart .and. 
     #        i.lt.beststart+bestcount+window-1) fixresidue(i)=.true.
      enddo
      write(0,*)"FIXED RESIDUES ", beststart, " TO ", 
     #     beststart+bestcount+window-2
