...
c some temporary and handshaking variables for hash
      ...
      real base_score,seq_score,bb_big_bin_score
...
c initialize heap
     ...
     #           seq_score,ss_scores,proline_phi_score,bb_big_bin_score,bad)

            if (.not.bad) then  ! flags gaps in vall plus cis omegas
               base_score = seq_score*seq_weight
     #              +proline_phi_score*weight_proline_phi+bb_big_bin_score


c force SS barcode
               if (force_ss(i).eq.ss(j)) then
