...
#' @author patrick
#' @export
class = function(data, class="", since_solved=F){
	indices=c()
	for(domain in unique(data$sequence_key)){
		if(!is.na(domain)){
			if(since_solved){
				query = "select concat(s.sccs,\".1\") from since_solved.since_solved s where s.cluster_id="||domain
			} else {
...
predict = function(data, class="a"){
	if(class == "a"){
		return(data[which(data$prediction_percent_alpha > 0.15 & data$prediction_percent_beta < 0.15), ])
	}
	if(class == "b"){
		return(data[which(data$prediction_percent_alpha < 0.15 & data$prediction_percent_beta > 0.15), ])
	}
...
#' @author patrick
#' @export
top = function(filter_id=2, experiment=NA, number=5, hpf_id=2, ids=NA, adjusted=FALSE, scale=0, since_solved=FALSE){

    if(since_solved){
	  query = since_solved_query(adjusted=adjusted, scale=scale, filter_id=filter_id)
	} else {
...
since_solved_query = function(filter_id=4, adjusted=FALSE,scale=0){
	if(adjusted){
	  table="hpf2.mcmdata_adjusted"
	  scale_q=" and scale="||scale
	  query = "SELECT m.*,s.cluster_id FROM  since_solved.since_solved s join "||table||" m on s.cluster_id = m.sequence_key where filter_id="||filter_id||scale_q||" order by s.cluster_id, probability desc;"
...
#' @author patrick
#' @export
correct = function(data, adjusted=FALSE, since_solved=FALSE){
	correct_indices = c()
	for(domain in unique(data$sequence_key)){
		if(!is.na(domain)){
			if(since_solved){
				#if(adjusted){
					query = "select s.sccs from since_solved.since_solved s where s.cluster_id="||domain
				#} else {
...
plot_mcm = function(data, since_solved=F, correct=T, breaks=40, main="Since Solved"){
	if(correct){
		return(plot_mcm_c(data, since_solved, breaks, main))
	} else {
		h = hist(data$prob, plot=F, breaks=breaks)
		plot(h$breaks[-1], h$counts, type="l", col="black", xlab="MCM Score", ylab="Frequency", main=main)
		return(h)
	}
...
plot_mcm_c = function(data, since_solved=F, breaks=40, main="Since Solved"){
	data_c = correct(data, since_solved=since_solved)
	data_i = data[which(!data$id %in% data_c$id), ]
	
	h_a = hist(data$prob, plot=F, breaks=breaks)
	h_c = hist(data_c$prob, plot=F, breaks=h_a$breaks)
	h_i = hist(data_i$prob, plot=F, breaks=h_a$breaks)
	
	# Calculate accuracy
	h_a$counts = h_c$counts/h_a$counts
	
	plot(h_i$breaks[-1], h_i$counts, type="l", col="black", xlab="MCM Score", ylab="Frequence", main=main)
	par(new=T)
	plot(h_c$breaks[-1], h_c$counts, type="l", col="red", axes=F, xlab=NA, ylab=NA)
	par(new=T)
	plot(h_a$breaks[-1], h_a$counts, type="l", col="blue", axes=F, ylim=c(0,1), xlab=NA, ylab=NA)
	axis(4)
	
	legend(0,1, c("Incorrect","Correct","Accuracy"), lwd=1, col=c("black","red","blue"))
