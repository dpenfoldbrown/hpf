"""
A copy and dev version of hpf.hddb.db py, for development work on HPF databases
dpb
6/22/2012

- New functionality should be tested here, and then moved into the production db.py ORM
- Points to the HPF development DB on markula.bio.nyu.edu
- Note: cleaning up a lot of extra functionality (may break some things)
"""
...
# HPF PRODUCTION LOCATION (DO NOT UNCOMMENT)
# url = "mysql://dpb:dpb_nyu@handbanana.bio.nyu.edu:3306/"
url = "mysql://dpb:dpb_nyu@markula.bio.nyu.edu:3306/"


# Connection utility functions (define engine, Base, Session, and ScopedSession below)
def clear():
...
def push_to_db(session, object, exception_str=None, raise_on_duplicate=True):
    ...
    if not exception_str:
        exception_str = "Failed to add given object to DB"
...
class SequenceGeneName(BaseStruct, Base):
    ...
    def __repr__(self):
        if self.db:
            return "<SequenceGeneName seq: {0} ac: {1} db: {2} symbol: {3}>".format(self.sequence_key, self.ac, self.db, self.symbol)
        else:
            return "<SequenceGeneName seq: {0} ac: {1} symbol: {3}>".format(self.sequence_key, self.ac, self.symbol)
...
class MouseGeneExprCorr(BaseStruct, Base):
    ...
    def __repr__(self):
        return "<Mouse gene expression data corr.: MGI Source: {0}, MGI Compared: {1}, Correlation: {2}>".format(self.mgi_src, self.mgi_cmp, self.correlation)
...
Psipred.sequence = relation(Sequence, primaryjoin="Sequence.id==Psipred.sequence_key", backref=backref("psipred",uselist=True,order_by=Psipred.timestamp.desc()))
...
class SCOPClass(BaseStruct, Base):
    ...
    description = relation(SCOPDescription, primaryjoin="and_(SCOPDescription.sccs==SCOPClass.sccs, SCOPDescription.entrytype.in_(['cl','cf','sf','fa','dm']))", uselist=False)
...
class Domain(BaseStruct, Base):
    ...
    def _best_astral(self):
        ...
        # Want to check overlapping astrals if there are mcm astrals but they don't meet cutoff
        if self.astral_overlap:
            best_astral = self.astral_overlap[0]
            for astral in self.astral_overlap[1:]:
                if astral.overlap > best_astral.overlap:
                    best_astral = astral
            if float(best_astral.overlap) >= self.ASTRAL_OVERLAP_CUTOFF:
                return best_astral
...
class DomainAstralOverlap(BaseStruct, Base):
    ...
    __table_args__ = (
            ...
            ForeignKeyConstraint(['astral_sid'], ['domain.id']),
            {'autoload':True}
            )
    domain = relation(Domain, primaryjoin="DomainAstralOverlap.domain_id==Domain.id", uselist=False, backref=backref("astral_overlap", uselist=True))
...
#            )
#    #__mapper_args__ = {"properties":{"sequence":relation(Sequence,primaryjoin="Sequence.id==Protein.sequence_key")}}
#    domain = relation(Domain, primaryjoin="FilesystemOutfile.sequence_key==Domain.domain_sequence_key")
#    protein = relation(Protein, primaryjoin="FilesystemOutfile.parent_sequence_key==Protein.sequence_key") 
...
class McmData(BaseStruct, Base):
    ...
    #__mapper_args__ = {"properties":{"sequence":relation(Sequence,primaryjoin="Sequence.id==Protein.sequence_key")}}
    domain = relation(Domain, primaryjoin="McmData.sequence_key==Domain.domain_sequence_key", uselist=False, backref=backref("mcmdata",uselist=True,primaryjoin="and_(McmData.sequence_key==Domain.domain_sequence_key,McmData.scop=='1.75')"))
    ...
    protein = relation(Protein, secondary=Domain.__table__, primaryjoin="McmData.sequence_key==Domain.domain_sequence_key",secondaryjoin="Domain.parent_sequence_key==Protein.sequence_key", backref="mcm")

    def __cmp__(self, other):
        return cmp(self.probability,other.probability)
    
    def __repr__(self):
        return "<McmData id:%i seq:%i sccs:%s conf:%f>" % (self.id, self.sequence_key, self.sccs, self.probability) 
...
class FilesystemOutfile(BaseStruct, Base):
    #    | parent_sequence_key | int(11)      | NO   | MUL |                   |                | 
    #| sequence_key        | int(11)      | NO   | MUL | 0                 |                | 
    #| executable_key      | int(11)      | NO   |     |                   |                | 
    #| fragment_key        | int(11)      | NO   | MUL |                   |                | 
...
class RosettaConvergence(BaseStruct, Base):
    ...
    cluster_centers = relation(Structure,secondary=RosettaCluster.__table__,primaryjoin="RosettaConvergence.id==RosettaCluster.convergence_key",secondaryjoin="RosettaCluster.structure_key==Structure.id",lazy=True,uselist=True,backref="convergence")
    outfile = relation(FilesystemOutfile,lazy=True)
    clusters = relation(RosettaCluster,lazy=True)
...
DomainAstralOverlap.astral = relation(Astral, primaryjoin="DomainAstralOverlap.astral_id==Astral.id", uselist=False, backref=backref("domain_overlap", uselist=True))
