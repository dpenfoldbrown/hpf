...
c  CVS information:
c  $Revision: 3351 $
c  $Date: 2003-09-04 20:36:21 -0400 (Thu, 04 Sep 2003) $
c  $Author: dylan $
...
c     data ss_type_quota/0.10,0.3,0.45,0.3,0.3,0.0/ !add jufo prediction
      ...
      write(0,*) 'ss_type_quotas must sum to 1.0 or larger'
      stop
      end
c---------------------------------------------------------------------
...
! intended to be an implicit equivalence to big_best_nn
      ...
      integer inserts_by_ssH, inserts_by_ssE, inserts_by_ssL
      integer total_inserts
      integer total_inserts_last, total_inserts_repeats, 
     #        total_inserts_repeats_max
      real ss_type_quota(max_ss_type)
...
c-------------------------------------------------------	 
      total_inserts_repeats_max = 20000
...
cdy   avoid looping forever at sparse positions (proline rich only?)
            total_inserts_last    = 0
            total_inserts_repeats = 0
...
cdy   avoid looping forever at sparse positions (proline rich only?)
                  if (total_inserts.eq.total_inserts_last) then
                     total_inserts_repeats = total_inserts_repeats + 1
                  else
                     total_inserts_repeats = 0
                     total_inserts_last = total_inserts
                  endif

                  if (total_inserts_repeats.ge.total_inserts_repeats_max) then
                     write (0,*) "inserts stuck at ",size+1,"mer pos: ",
     #                           i," inserts: ", total_inserts

                     if (total_inserts.lt.25) then
                        write (0,*) "shockingly low fragment diversity ",
     #                              "for ",size+1,"mers at pos: ", i,
     ...
     #                              "  exiting..."
                     ...
                     do k2=0, max_nn, 1
                        do k=1, total_inserts, 1
                           write (0,*)"copying frag ", k, " into frag ",
     #                                total_inserts+k+k2*total_inserts, 
     #                               " at pos:", i
                           b_nn(size,i,total_inserts+k+k2*total_inserts)
     #                               = b_nn(size,i,k)
                           if (total_inserts+k+k2*total_inserts.eq.max_nn) 
     #                               goto 100
                        enddo
                     enddo

 100                 total_inserts=max_nn
                  endif

               endif ! if (ss_type_quota(ss_type).gt.0.0.and.pointer_by_ss_type(ss_type).lt.max_nn)
