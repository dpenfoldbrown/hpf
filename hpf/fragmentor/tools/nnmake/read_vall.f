...
c  CVS information:
c  $Revision: 6809 $
c  $Date: 2005-10-11 13:26:48 -0400 (Tue, 11 Oct 2005) $
c  $Author: tex $
...
C  also it accepts the format decided on by DChivian when we remade the vall 
C	RBonneau , 4/2000
C----------------------------------------------------------------------
      ...
      filename=read_vall_path(1:read_vall_l)//vall_name
      write(0,*) 'read_vall (unit=',iunit,'): ',
     #     filename(1:index(filename,' '))
      open(iunit,file=filename(1:index(filename,' ')),status='old',iostat=fail)
      if (fail.ne.0) then
         filename=read_vall2_path(1:read_vall2_l)//vall_name
         write(0,*) 'read_vall (unit=',iunit,'): ',
     #        filename(1:index(filename,' '))
         open(iunit,file=filename(1:index(filename,' ')),status='old',iostat=fail)
         if (fail.ne.0) then
...
c end test will catch this case and everybody is cool
      ...
      write(0,*)"increase max_vall in param.h"
      stop
