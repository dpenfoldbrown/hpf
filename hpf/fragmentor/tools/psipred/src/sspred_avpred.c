...
predict(int argc, char **argv)
{
    ...
    REAL            avout[MAXSEQLEN][3];

    for (winpos = 0; winpos < seqlen; winpos++)
	avout[winpos][0] = avout[winpos][1] = avout[winpos][2] = 0;
	...
	for (winpos = 0; winpos < seqlen; winpos++)
	  {
	    ...
	    avout[winpos][0] += activation[TOTAL - NUM_OUT];
	    avout[winpos][1] += activation[TOTAL - NUM_OUT+1];
	    avout[winpos][2] += activation[TOTAL - NUM_OUT+2];
	  }
      }
    
    for (winpos = 0; winpos < seqlen; winpos++)
      {
	avout[winpos][0] /= (argc-2);
	avout[winpos][1] /= (argc-2);
	avout[winpos][2] /= (argc-2);
