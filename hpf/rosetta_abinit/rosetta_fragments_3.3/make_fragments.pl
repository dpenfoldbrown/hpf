...
local $BLAST_DIR = "/scratch/dpb3/ginzu/opt/blast-2.2.25";
...
local $VALL = "/home/dpb3/rosetta/database/vall.apr24.2008.extended";

# ROSETTA PICKER
local $PICKER =
  "/share/apps/rosetta/RosettaReleases/rosetta-3.3/rosetta_source/bin/fragment_picker.default.linuxgccrelease";
local $ROSETTA_DATABASE =
  "/share/apps/rosetta/RosettaReleases/rosetta-3.3/rosetta_database";
...
local $PSIPRED_DIR =
  "/scratch/dpb3/ginzu/opt/psipred";    # psipred installation directory

# pfilt filtered NR blast database filename used for PSIPRED (see PSIPRED readme)
local $PFILTNR = "/data/nr/nr_pfilt";


##
## IGNORING: SAM and PORTER.
## Always run with '-nosam' and '-noporter' options
##
...
if ($no_homs) {
    foreach my $vall (@vall_files) {
        ...
        unless ( &fileExist("$vallname.blast.outn") ) {
            if (
                !&try_try_again(
"$BLASTPGP -t 1 -i $file -j 1 -R $id$chain.nr.chk -o $vallname.blast.outn -e 0.05 -d $vall",
                    2,
                    ["$vallname.blast.outn"],
                    ["$vallname.blast.outn"]
                )
