...
long int numResidues;
int numHomologs,NATIVE,numDecoys,numGoodCoords,
...
int ** DoClustering( float ***neighbors, float threshold, int target, int clusterCenter, 
  ...
  float **clusterList,**nl;
	...
	fprintf(stderr, "In DoClustering\n");
  /** setup **/
  ...
  for (i=0;i<numDecoys;i++) exists[i] = 1;
	...
	fprintf(stderr, "done initializing exists\n"); 
  ...
  for (i=0;i<numDecoys;i++) {
	...
	if(NULL == clusterList[i])
	{ fprintf(stderr, "calloc memory error\n"); }
  }
	fprintf(stderr, "done initializing clusterList\n"); 
  ...
  for (i=0;i<MAX_NUM_CLUSTERS;i++)
	...
	fprintf(stderr, "done initializing clusterMembers\n");
  ...
  while (1) { /** make clusters in this loop -- break when size is < minClusterSize **/
	...
	fprintf(stderr, "after if clusterCount\n");
	...
	if(NULL == neighbors)
	{fprintf(stderr, "neighbors is NULL\n");}

	fprintf(stderr, "DoClustering - clusterCenter: %i\n", clusterCenter);
	

    nl = neighbors[clusterCenter];
	fprintf(stderr, "before for target\n");

	for (i=0;i<target;i++) 
	{
		fprintf(stderr, "before nl[i]\n");

		if(NULL == nl)
		{fprintf(stderr, "nl is NULL\n");}
		if(NULL == nl[i])
		{fprintf(stderr, "nl[i] is NULL\n");}
		//if(NULL == nl[i][0])
		//{fprintf(stderr, "nl[i][0] is NULL\n");}
		//if(NULL == nl[i][1])
		//{fprintf(stderr, "nl[i][1] is NULL\n");}

		if (nl[i][0] >= threshold) break;

		fprintf(stderr, "after threshold\n");

		if (! exists[ (int) nl[i][1]]) continue;

		fprintf(stderr, "after exists\n");

		clusterMembers[clusterCount][count+1] =  (int) nl[i][1];

		fprintf(stderr, "after clusterMembers\n");

		exists[ (int) nl[i][1] ] = 0;

		fprintf(stderr, "after exists = 0\n");

		count++;
		fprintf(stderr, "after count\n");
	}
	fprintf(stderr, "after for target\n");
	...
	fprintf(stderr, "before update clusterList\n");
    /** Update clusterList **/
	...
	fprintf(stderr, "after update clusterList\n");
...
void MakeOutput (char *filename, float threshold, int target, 
		 ...
		 int top_size1, float top_threshold1,
		 int top_size2, float top_threshold2 ) {
  ...
  fprintf(file,"standard_thresholds: size1= %d threshold1= %f size2= %d threshold2= %f total_decoys= %d\n",
	  top_size1, top_threshold1, top_size2, top_threshold2, numDecoys ); 
...
int main(int argc, char *argv[]) {
  ...
  // diagnostics:
  int top_size1, top_size2;
  float top_threshold1, top_threshold2;
  ...
  /** want to output some data on clustering thresholds for Lars' confidence function:**/
  top_size1 = (int) ( numDecoys * 0.02 );
  top_size2 = (int) ( numDecoys * 0.05 );
  if ( top_size2 > maxClusterSize ) {
    fprintf(stderr,"WARNING:: setting maxClusterSize from %d to %d for diagnostics\n",
	    maxClusterSize, top_size2 );
    maxClusterSize = top_size2;
  }

  if ( top_size1 < 1 ) {
    fprintf(stderr,
	    "WARNING:: too few decoys for diagnostics, setting top_size1=4,top_size2=10\n");
    top_size1 = ( 4 <= numDecoys ? 4 : numDecoys );
    top_size2 = ( 10 <= numDecoys ? 10 : numDecoys );
  }
  ...
  while (!done) { /** keep looping until we have threshold >= minThreshold, probably only once*/
    ...
    // for diagnostics:
    top_threshold1 = ClusteringThreshold( neighbors, numDecoys, 
					  top_size1-1 ,top_size1, top_size1+1,
					  0,1000.0,
					  &clusterCenter,
					  &target,0);
      fprintf(stderr, "clusterCenter1: %i\n", clusterCenter);
    top_threshold2 = ClusteringThreshold( neighbors, numDecoys, 
					  top_size2-1 ,top_size2, top_size2+1,
					  0,1000.0,
					  &clusterCenter,
					  &target,0);
      fprintf(stderr, "clusterCenter2: %i\n", clusterCenter);
	...
	/*kdrew: added for debugging*/
      fprintf(stderr, "finished iteration of ClusteringThreshold\n");
    if ( (threshold < minThreshold) && (maxClusterSize<numDecoys-1) ) {
	/*kdrew: added for debugging*/
      fprintf(stderr, "threshold < minThreshold and maxClusterSize < numDecoys-1\n");
      FreeNeighbors(neighbors,numDecoys,maxClusterSize);
      fprintf(stderr, "done freeing neighbors\n");
  ...
  MakeOutput (outputFilename, threshold, target, clusterMembers, numClusters,
	      top_size1, top_threshold1, top_size2, top_threshold2);
